...
class CategoryController {
    ...
    show = async (req, res, next) => {
        const category = await CategoryModel.findByPk(req.params.cityId);
        res.json(category);
    }
    ...
    _validateData = async (data, id) => {
        ...
        if (await this._categoryExists(category.name, id)) {
            throw new Error(`The category with name "${category.name}" already exists.`);
        }
        return category;
    }
    _categoryExists = async (name, id) => {
        const where = {
            name: name
        };
