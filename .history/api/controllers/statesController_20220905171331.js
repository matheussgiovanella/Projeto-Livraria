...
class StateController {
    ...
    _validateData = async (data, id) => {
        ...
        if (await this._stateExists(state.name, id)) {
            throw new Error(`The state "${state.name}" already exists.`);
        }
        if (await this._stateExists(state.province, id)) {
            throw new Error(`The state "${state.province}" already exists.`);
        }
        return state;
    }
    _stateExists = async (param, id) => {
        const where = {
            param: param
        };
