...
class PublisherController {
    ...
    create = async (req, res, next) => {
        try {
            ...
            const action = `Publisher ${publisher.dataValues.name}-${city.dataValues.name}/${state.dataValues.province} created`;
    ...
    update = async (req, res, next) => {
        try {
            ...
            const newCity = await City.findByPk(oldPublisher.dataValues.city_id);
            const newState = await State.findByPk(oldCity.dataValues.state_id);
            const action = `Publisher ${oldPublisher.dataValues.name}-${oldCity.dataValues.name}/${oldState.dataValues.province}` +
                `->`
