...
const showMessage = async (message) => {
    const popUp = new popUpOBJ;
    let popUpMessage = '';
    popUpMessage += `<span class="popUp-title">${message}</span><br/>`;
...
const showPopUp2 = async (message) => {
    let popUp = '';
    popUp += `<div class="popUp2">`;
    popUp += `<div class="popUp2-container">`;
    popUp += `<button class="popUp2-button" onclick="closePopUp2()">X</button>`;
    popUp += `${message}`;
    popUp += `</div>`;
    popUp += `</div>`;
    document.querySelector('body').innerHTML += popUp;
