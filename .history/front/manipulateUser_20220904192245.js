...
const showPopUp = async (email) => {
    ...
    popUp += `<div><input id="password"/>teste</div>`;
    popUp += `<div><button onclick="confirmPassword(document.querySelector('#password').value)">Confirm</button></div>`;
    popUp += `</div>`;
    popUp += `</div>`;
...
const confirmPassword = async (password) => {
    ...
    password = await encryptPassword(password);
    response = await axios.get(`${ENDPOINT}/users?password=${password}`);
