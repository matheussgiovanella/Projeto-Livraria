...
const loadTable = async () => {
    ...
    users.forEach(element => {
        ...
        trHTML += `<button onclick="showPopUp('${element.name}')">Del</button></td>`;
        trHTML += `</tr>`;
    });
...
const showPopUp = async (name) => {
    ...
    popUp += `<label for="password">Confirm Password for ${name}</label>`;
    popUp += `<div><input id="password"/></div>`;
    popUp += `<div><button onclick="confirmPassword('${name}')">Confirm</button></div>`;
    popUp += `</div>`;
    popUp += `</div>`;
...
const confirmPassword = async (name) => {
