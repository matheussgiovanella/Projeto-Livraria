...
const loadTable = async () => {
    ...
    users.forEach(element => {
        ...
        trHTML += `<td class="buttons"><button onclick="confirmUserForm('${element.id}')">Edit</button>`;
        trHTML += `<button onclick="confirmUserForm('${element.id}')">Del</button></td>`;
        trHTML += `</tr>`;
    });
...
const confirmUserForm = async (id) => {
    Swal.fire({
        title: 'Edit User',
        html:
            `<input id="password" class="swal2-input" placeholder="Name" value= ${data.id} >`,
        focusConfirm: false,
