...
const editBookForm = async (id) => {
    ...
    Swal.fire({
        ...
        html:
            ...
            `<select id="category"><option value="${category.id}" selected>${category.description}</option>${CategoryOptionHTML}</select>` +
            `<select id="publisher"><option value="${publisher.id}" selected>${publisher.name}</option>${PublisherOptionHTML}</select>`,
        focusConfirm: false,
        showCancelButton: true,
        preConfirm: async () => {
            await updateBook(id);
