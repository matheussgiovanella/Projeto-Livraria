...
const editUserForm = async (id) => {
    const state = await getState(id);
    Swal.fire({
        title: `Edit ${state.email.split(" ").join("")}`,
        html:
            `<input id="id" type="hidden" value=${state.id}>` +
            `<input id="name" class="swal2-input" placeholder="Name" value="${state.name.split(" ").join("")}">` +
            `<input id="province" class="swal2-input" placeholder="Province" value="${state.province.split(" ").join("")}">`,
        focusConfirm: false,
        showCancelButton: true,
        preConfirm: async () => {
