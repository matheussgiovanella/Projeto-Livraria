    ...
    for (const city of cities) {
        for (const state of states.data) {
            if (state.id == city.state_id) {
                optionHTML += `<option value="${city.id}">${city.name}/${state}</option>`;
            }
        }
    }
    return optionHTML;
...
const editPublisherForm = async (id) => {
    ...
    Swal.fire({
        ...
        html:
            ...
            `<select class="swal2-input" id="city"><option value="${city.id}" selected>${city.name}</option>${optionHTML}</select>`,
        focusConfirm: false,
        showCancelButton: true,
        preConfirm: async () => {
...
const deletePublisher = async (id) => {
    const publisher = await getPublisher(`${ENDPOINT}/publishers/${id}`);
