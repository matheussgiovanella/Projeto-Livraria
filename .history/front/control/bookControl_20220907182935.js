...
const editBookForm = async (id) => {
    ...
    const optionHTML = await getCategories(category_id) + await getPublishers(publisher_id);
    Swal.fire({
        title: `Edit ${book.title}`,
        html:
            `<input id="id" type="hidden" value=${book.id}>` +
            `<input id="title" class="swal2-input" placeholder="Title" value="${publisher.name}">` +
