...
const loadTable = async () => {
    ...
    users.forEach(element => {
        ...
        trHTML += `<button onclick="showPopUp('${element.email}')">Del</button></td>`;
        trHTML += `</tr>`;
    });
...
const preparePopUpMessage = async (param) => {
    let message = '';
    message +=`<label for="password">Confirm Password for ${email}</label>`;
    message += `<div><input id="password"/></div>`;
    message += `<div><button onclick="confirmPassword('${email}')">Confirm</button></div>`;

    showPopUp(message);
}

const showPopUp = async (email) => {
    let popUp = '';
    popUp += `<div class="popUp">`;
    popUp += `<div class="popUp-container">`;
    popUp += `<button class="popUp-button" onclick="closePopUp()">X</button>`;
    popUp += `<label for="password">Confirm Password for ${email}</label>`;
    popUp += `<div><input id="password"/></div>`;
    popUp += `<div><button onclick="confirmPassword('${email}')">Confirm</button></div>`;
    popUp += `</div>`;
    popUp += `</div>`;
